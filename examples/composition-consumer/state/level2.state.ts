...
export class Level2State extends AlarmState {
  ...
  public change(alarm: Alarm<Composition>): void | Promise<void> {
    ...
    if (current - alarm.timestamp >= this._timeBounce) {
      // 溫度超過 30 度且濕度超過 70%，持續 5 秒鐘，升級報警至 L1
      this.logger.info(
        `${alarm.key} temperature: ${temperature} over ${this._temperature} and humdity: ${humidity} over ${this._humidity} in 5sec update to L2`,
      );
